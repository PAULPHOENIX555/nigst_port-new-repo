...
function handleInputs(e){
  ...
  const otpUrl = "http://ec2-13-233-110-121.ap-south-1.compute.amazonaws.com/sms/so";
    const data={
      fname:`${input.fname.charAt(0).toUpperCase() + input.fname.slice(1)}`,
      mname:`${input.mname.charAt(0).toUpperCase() + input.fname.slice(1)}`,
      lname:`${input.lname.charAt(0).toUpperCase() + input.fname.slice(1)}`,
      dob:`${input.dob}`,
      phone:`${input.phone}`,
      gender:`${gender}`,
      email:`${input.email}`,
